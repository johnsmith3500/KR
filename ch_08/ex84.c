/*
 * ex84.c - The standard library function
 * int fseek(FILE *fp, long offset, int origin)
 * is identical to lseek except that fp is a file pointer instead of 
 * a file descriptor and  the return value is an int status, not a position. 
 * Write fseek . Make sure that your fseek coordinates properly 
 * with the buffering done for the other functions of the library. 
 *
 * programm 8.2 and 8.3 and 8.4 from k&r; don't include stdio.h; input.txt: abcdefghijk
...
int fseek(FILE *fp, long offset, int origin)    // set position
{
    ...
    {    
        if(lseek(fp->fd, offset-(fp->cnt), SEEK_CUR)<0)    /* real (system) ptr is not as fp->ptr */
            return -1;
    }
    else
...
int main(int argc, char *argv[])
{
    ...
    /* test SEEK_SET for reading */
    fseek(fpi, 0, SEEK_SET);
    c=getc(fpi);
    if(c==EOF)
        putc(EOF, fpo1);
    putc(c, fpo1);
    /* test SEEK_END for reading (at the end of a test input file may be /n) */
    fseek(fpi, -2, SEEK_END);
    c=getc(fpi);
    if(c==EOF)
        putc(EOF, fpo1);
    putc(c, fpo1);
    /* test SEEK_CUR for writing */
    fseek(fpo1, -4, SEEK_CUR);
    putc('z', fpo1);
    /* test SEEK_SET for writing */
    fseek(fpo1, 0, SEEK_SET);
    putc('z', fpo1);
    /* test SEEK_end for writing */
    fseek(fpo1, -2, SEEK_END);
    putc('z', fpo1);
